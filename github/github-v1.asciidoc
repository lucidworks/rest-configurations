...
* Properties `Response Handling -> Data ID` is configured to extract unique identifiers from the objects parsed. For most entities, `html_url` provides a globally unique, human-readable URL.
...
* Properties `Response Handling -> Parent Data Key` is required by the connector on all child requests. It specifies which field to extract from the parent response object; that value replaces `${LW_PARENT_DATA_KEY}` in the child request's endpoint or query parameters.
...
** and the BLOB request uses `parentIdKey=path` to extract the file path from its parent FOLDER object.
