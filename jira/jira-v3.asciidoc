...
This documentation describes aspects of the Jira REST `jira-v3.json` file configuration such as the authentication methods, endpoints requested, data crawled, pagination information. Terminology is also provided as a reference.
...
The configuration uses the Jira API v3.0 https://developer.atlassian.com/cloud/jira/platform/rest/v3/intro/#version

NOTE: This configuration was migrated from v2 to v3 due to Atlassian permanently removing Jira Cloud REST API v2 endpoints (CHANGE-2046). The `/rest/api/2/search` endpoint now returns HTTP 410 Gone. See the https://lucidworks.atlassian.net/wiki/spaces/FENG/pages/3972792321[Jira API Migration Guide] for details.
...
For each request-configuration, configure the property *Response Handling* to set up how to parse the response (`responseConfiguration` in the `jira-v3.json` file)
...
* Enable by setting the property `Response Handling -> Parse Binary Data` (`binaryResponse` in the `jira-v3.json` file). Send the whole response to the Fusion Parsers. If disabled (default), the response is parsed as a JSON object
...
id: "serverURL_/rest/api/3/issue/issueID_attachmentID",
filename_s: "file.txt",
...
id: "serverURL_/rest/api/3/attachment/content/attachmentID_attachmentID_binary",
filename_s: "file.txt",
mimeType_s: "text/plain",
body_s: "body of txt"
_lw_rest_object_type_s: "attachment_download"
```
- There is no need to index the first solr-doc. To avoid indexing this, the property *'Skip Indexation'* for the Request ATTACHMENT_METADATA is enabled in the 'jira-v3.json'.
