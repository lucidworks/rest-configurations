...
This documentation describes aspects of the Confluence REST `confluence.json` file configuration such as the authentication methods, endpoints requested, data crawled, pagination information. Terminology is also provided as a reference.
...
The deprecated Confluence v1 REST API endpoints required to obtain the object mentioned above have been gradually phased out over the years 2025 and 2026.
For more information, see https://community.developer.atlassian.com/t/update-to-confluence-v1-api-deprecation-timeline/79687/18[v1-api-deprecation-timeline]
and https://developer.atlassian.com/cloud/confluence/changelog/#.

Therefore the outdated configuration file, which depended on v1 endpoints, should not be used.
...
* Each requests in configured under the property *List of Requests Configuration* (`requestConfigurations` in the confluence.json` file)
...
Per request, configure the property *Response Handling* to set up how to parse the response (`responseConfiguration` in the `confluence.json` file)
...
=== Binary Parsing:
* This property is not used with the `confluence.json` configuration.
